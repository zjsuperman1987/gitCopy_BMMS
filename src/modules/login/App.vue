<template>
	<div id="app" v-show="IsShow">
		...
		<div class="content">
			<div>
				<input type="text" v-model="MemerPhone" placeholder="请输入你的手机号" class="login_input">
			</div>
			<div>
				<input type="text" v-model="ValidCode" placeholder="请输入验证码" class="login_input" style="width: 60%">
				<button @click="BindValidCode()" style="float: right; width: 35; margin-top:10px;" v-bind:disabled="dis">
					{{btn_ValidCode}}
				</button>
			</div>
			...
			BindMobilePhone: function() {
				...
				if(this.ValidCode == null || this.ValidCode.trim() == "") {
					...
					if(RegCellPhone.test(this.MemberPhone)) {
						ajax.query(this, 'BindMobilPhone', {
							"openid": vue.openid,
							"access_token": vue.access_token,
							"MobilPhone": vue.MemberPhone
						}).then((res) => {
